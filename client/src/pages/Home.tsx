...
export default function Home() {
  ...
  const view = workflowViews[window.location.pathname as keyof typeof workflowViews] ?? "overview";
  ...
  const currentCampaign = data?.campaigns[0];
  ...
  const loadDemo = async () => {
    ...
    toast.success("Synthetic training workspace is ready.");
  };
  ...
  const beginCall = async () => {
    ...
    try {
      ...
      const opening = { role: "agent" as const, content: result.opening, source: "Hard-coded disclosure + approved opening" };
  ...
  if (!data || data.campaigns.length === 0) return <SyntheticOnboarding loading={seedDemo.isPending} onLoad={loadDemo} />;
  ...
  const policyContent = policy ? JSON.parse(policy.policyJson) as { label: string; permissions: string[]; blocks: string[] } : null;

  return (
    <div className="mx-auto max-w-[1440px] pb-10 text-[#10213b]">
      <header className="mb-7 border-b border-[#dce4ee] pb-6">
        <div className="flex items-center gap-2 text-xs font-semibold text-[#0d6e6e]"><span className="inline-flex h-2 w-2 rounded-full bg-[#2ea89d]" />COLLEGE OUTREACH DEMO</div><h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em] sm:text-[34px]">Admissions outreach, ready to explore.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#5c6d83]">A browser-only conversation simulator for a fictional college, using synthetic contacts and sample course information.</p>
      </header>
      ...
      {view === "inbound" || view === "delegated" ? <WorkflowPolicyView type={view} policy={policyContent} /> : view === "contacts" ? <ContactsView contacts={contacts} records={records} addContact={addContact} bulkAdd={bulkAdd} csvPreview={csvPreview} setCsvPreview={setCsvPreview} handleCsv={handleCsv} dialogOpen={contactDialogOpen} setDialogOpen={setContactDialogOpen} /> : <>
        ...
        <section className="mt-7 grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(320px,0.8fr)]">
          <div className="rounded-2xl border border-[#dce4ee] bg-white">
            ...
            <div className="grid border-b border-[#e6ecf2] lg:grid-cols-[1fr_auto] lg:items-end"><div className="p-5"><p className="text-xs font-semibold tracking-[0.08em] text-[#6e7f94]">ACTIVE CAMPAIGN</p><div className="mt-2 flex flex-wrap items-center gap-2"><h3 className="text-base font-semibold">{currentCampaign.name}</h3><Badge className={currentCampaign.status === "approved" ? "bg-[#def5ee] text-[#075f59] hover:bg-[#def5ee]" : "bg-[#fff1d9] text-[#8b5e1a] hover:bg-[#fff1d9]"}>{currentCampaign.status === "approved" ? "Approved" : "Approval needed"}</Badge></div><p className="mt-2 text-xs leading-5 text-[#6a7b90]">9am–9pm IST · frequency cap {currentCampaign.frequencyCap} · approved knowledge base attached</p></div>{currentCampaign.status !== "approved" && <div className="px-5 pb-5 lg:pl-0"><Button onClick={() => approveCampaign.mutate({ campaignId: currentCampaign.id })} disabled={approveCampaign.isPending} className="bg-[#0d6e6e] text-white hover:bg-[#095d5d]">{approveCampaign.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Approve campaign</Button></div>}</div>
...
function CallSimulator({ contact, campaignApproved, callState, transcript, activeLanguage, message, setMessage, pendingOutcome, summary, starting, responding, finishing, onStart, onSend, onFinish, onReset, transcriptEnd }: { contact: any; campaignApproved: boolean; callState: "idle" | "active" | "finishing" | "complete"; transcript: TranscriptItem[]; activeLanguage: "English" | "Hindi"; message: string; setMessage: (value: string) => void; pendingOutcome: Outcome; summary: string; starting: boolean; responding: boolean; finishing: boolean; onStart: () => void; onSend: (event: FormEvent) => void; onFinish: () => void; onReset: () => void; transcriptEnd: React.RefObject<HTMLDivElement | null> }) {
